...
FISCH_TEST_NUMBER_REGISTER_GENERAL(I2CTempRegister, 0, 0x1234)
FISCH_TEST_NUMBER_REGISTER_CEREAL(I2CTempRegister, 0x1234)

TEST(I2CTempRegister, EncodeRead)
{
	...
	auto addr_write = UTMessageToFPGA<instruction::omnibus_to_fpga::Address>(
	    instruction::omnibus_to_fpga::Address::Payload(i2c_tmp112_base_address, false));

	auto addr_read = UTMessageToFPGA<instruction::omnibus_to_fpga::Address>(
	    instruction::omnibus_to_fpga::Address::Payload(i2c_tmp112_base_address, true));

	auto addr_read_stop = UTMessageToFPGA<instruction::omnibus_to_fpga::Address>(
	    instruction::omnibus_to_fpga::Address::Payload(
	        i2c_tmp112_base_address | i2c_over_omnibus_stop, true));
	...
	EXPECT_EQ(
	    message_data_1, UTMessageToFPGA<instruction::omnibus_to_fpga::Data>(
	                        instruction::omnibus_to_fpga::Data::Payload(0x00)));
	auto message_addr_2 =
...
TEST(I2CTempRegister, Ostream)
{
	I2CTempRegister obj;
	obj.set(I2CTempRegister::Value(0xa));

	std::stringstream stream;
	stream << obj;

	EXPECT_EQ(stream.str(), "I2CTempRegister(0d10 0xa 0b0000000000001010)");
}
